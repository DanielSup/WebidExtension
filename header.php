...
$usersCountry = isset($user->user_data['country_id']) ? $user->user_data['country_id'] : '';

$countriesQuery = "SELECT * FROM ". $DBPrefix . "countries;";
$db->query($countriesQuery);
$options = '';
while ($result = $db->result()){
	$selected = $result['country_id'] == $usersCountry ? 'selected' : '';
	$options .= '<option value="'. $result['country_id'] . '" '. $selected .'>'. $result['country'] .'</option>';
}
