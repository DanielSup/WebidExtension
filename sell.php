...
switch ($_SESSION['action']) {
    ...
    case 3: // confirm auction
        ...
        if ($noerror) {
            ...
            foreach($_SESSION['countries'] as $country){
                ...
                $shipping_options_list = '<ul>';
                $shipping_options = $country['shipping-options'];
                foreach($shipping_options as $shipping_option){
                    if($shipping_option['id'] == ''){
                        break;
                    }

                    $shipping_option_item = '<li>';
                    $shipping_option_item .= '<p>' . $shipping_option['id'] . '</p>';
                    $shipping_option_item .= '<p>' . $shipping_option['title'] . '</p>';
                    $shipping_option_item .= '<p>' . $shipping_option['shipping-for-first-item'] . ' CZK</p>';
                    $shipping_option_item .= '<p>' . $shipping_option['shipping-for-second-item'] . ' CZK</p>';
                    $shipping_option_item .= '</li>';
                    $shipping_options_list .= $shipping_option_item;
                }
    ...
    case 1:  // enter auction details
        ...
        while($country = $db->fetch()){
            ...
            $possible_countries[$country_id] = $country;
        }

        $query = "SELECT * FROM " . $DBPrefix. "shipping_options";
        $db->direct_query($query);
        $possible_shipping_options = array();
        while($shipping_option = $db->fetch()){
            $id = $shipping_option['id'];
            $name = $shipping_option['name'];
            $possible_shipping_options[$id] = $name;
        }
        ...
        if (isset($_SESSION['countries'])){
            ...
            foreach($countries as $id => $country){
                ...
                $selected_country = "<div id='country-definition-$first_country_to_select' class='form-group col-md-12 dutchhide'>";
                ...
                $selected_country .= "<p>$country_select</p>";

                $selected_country .= "<legend>$country_name</legend>";

                # rendering filled fields for shipping options
                $first_shipping_option_to_select = 1;
                $shipping_options = $country['shipping-options'];
                $available_shipping_options = $possible_shipping_options;
                foreach($shipping_options as $shipping_option_index => $shipping_option) {
                    if ($shipping_option['id'] == '') {
                        $selected_country .= addShippingOption($first_country_to_select, $first_shipping_option_to_select, $available_shipping_options, null);
                        break;
                    }

                    $shipping_option_id = $shipping_option['id'];
                    $shipping_option_title = $possible_shipping_options[intval($shipping_option_id)];
                    $selected_country .= addShippingOption($id, $shipping_option_index, array($shipping_option_id => $shipping_option_title), $shipping_option);
                ...
                unset($possible_countries[$country_id]);
                $first_country_to_select++;
            }
        }
        ...
        $db->direct_query($query);
        while($shipping_option = $db->fetch()){
            $shipping_options .= "<option value='" . $shipping_option['id'] . "'>" . $shipping_option['name'] . "</option>";
        }
        ...
        $template->assign_vars(array(
                ...
                'HANDLE_SHIPPING_OPTIONS_FOR_SELECTED_COUNTRIES' => $handle_shipping_options_for_selected_countries,
                'SELECTED_COUNTRIES' => $selected_countries,
                'FIRST_COUNTRY_ORDER_TO_SELECT' => $first_country_to_select,
